 ...
 ***********************************/
const CONFIG = {
  // CRM API (primary submission)
  CRM_API_URL: '/api/ingest/lead-form',

  // Airtable (backup submission)
...
async function handleSubmit(evt) {
  ...
  try {
    ...
    try {
      ...
      console.log('CRM submission completed successfully');
    } catch (error) {
      console.error('CRM submission failed, but continuing with backup Airtable:', error);
      // Continue with Airtable even if CRM fails
    }

    // Backup submission to Airtable
    try {
      console.log('Calling submitToAirtable...');
      // Include CRM result in Airtable submission if available
      if (crmResult && crmResult.data) {
        state.trackingData.crmCustomerId = crmResult.data.customerId;
        state.trackingData.crmLeadId = crmResult.data.leadId;
      }
      airtableResult = await submitToAirtable();
      console.log('Airtable backup submission completed');
    } catch (error) {
      console.error('Airtable backup submission failed:', error);
      // Only fail completely if both CRM and Airtable fail
      if (!crmResult) {
        throw new Error('Both CRM and Airtable submissions failed');
      }
    }

    console.log('Dual submission completed, tracking conversion...');
...
/**
 * Submit to CRM API (primary submission)
 */
async function submitToCRM() {
  console.log('=== CRM SUBMISSION START ===');
  console.log('Building CRM payload...');

  // Build CRM payload matching the API schema
  const payload = {
    fullName: state.formData.fullName,
    email: state.formData.email,
    phone: qs('phone').value.replace(/\D/g, ''), // Clean phone number
    countryCode: qs('countryCode').value,
    ageVerification: state.formData.ageVerification,
    promotionalConsent: state.formData.promoConsent,

    // Verification status
    emailVerified: state.formData.emailVerified,
    phoneVerified: state.formData.smsVerified,
    ...
    landingPage: state.trackingData.landingPage,
    redirectUrl: state.trackingData.redirectUrl,
    ...
    platform: state.geoData.platform || '',

    // Timestamp
    timestamp: state.trackingData.timestamp || new Date().toISOString(),
  };

  console.log('CRM payload built:', payload);
  debugLog('CRM payload', payload);
  ...
  try {
    ...
    debugLog('CRM submission error', error);
    throw error;
  }
}

/**
 * Submit to Airtable (backup submission)
 */
async function submitToAirtable() {
  ...
  try {
    ...
    if (CONFIG.PROXY_URL && CONFIG.PROXY_URL.trim() !== '') {
      console.log('Using proxy URL for Airtable submission...');
      ...
      if (!response.ok) {
        ...
        console.error('Airtable proxy submission failed:', response.status, errorData);
        throw new Error(`Airtable Proxy HTTP ${response.status}: ${JSON.stringify(errorData)}`);
      }

      result = await response.json();
      console.log('Airtable proxy submission successful:', result);
    } else {
      ...
      result = await response.json();
      console.log('Direct Airtable submission successful:', result);
    }
    ...
    debugLog('Airtable submission error', error);
    throw error;
  }
}
